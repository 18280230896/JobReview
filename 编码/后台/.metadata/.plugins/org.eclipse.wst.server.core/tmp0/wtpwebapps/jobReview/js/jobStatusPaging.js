...
function init(){
	//初始化当前页数
	nowPage = 1;
	if(type == 1) getCount("teacherGetStudentCount.action");
	else getCount("teacherGetGroupTotal.action");
	showPage(nowPage);
	createTabs();
}
...
function showPage(page){
	nowPage = page;
	getData("teacherGetTaskSubmitStatus.action");
	createTable();
}
...
function getData(url){
	$.ajax({
		url:url,
		type:"post",
		data:{"ctid":$("#ctid").val(),"startNum":(nowPage-1)*pageSize,"pageSize":pageSize},
		dataType:"json",
		async:false,
		success:function(result){
			if(result.status == 1){
