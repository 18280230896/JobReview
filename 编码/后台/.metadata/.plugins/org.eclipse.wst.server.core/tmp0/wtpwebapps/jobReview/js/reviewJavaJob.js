...
$(".leftBtn").click(function(){
	if(show){
		$(".review-box").animate({"right":"-250px"},250);
		show = !show;
		$(".leftBtn span").removeClass("glyphicon-triangle-right").addClass("glyphicon-triangle-left");
	}else{
		$(".review-box").animate({"right":"0px"},250);
		show = !show;
		$(".leftBtn span").removeClass("glyphicon-triangle-left").addClass("glyphicon-triangle-right");
	}
});
...
$(".nextSubject").click(function(){
	...
	var width = $(".content")[0].offsetWidth;
	$(".content-list").animate({"left":-subjectIndex*width},400);
	if(subjectIndex == task.subjects.length - 1){
		$(".nextSubject").prop("disabled","disabled");
		if(!show) $(".leftBtn").click();
	}
